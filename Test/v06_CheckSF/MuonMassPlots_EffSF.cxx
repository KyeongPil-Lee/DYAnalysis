...
void MuonMassPlots_EffSF(Bool_t isCorrected = kTRUE, TString Type = "MCBkg", TString HLTname = "IsoMu20_OR_IsoTkMu20")
{
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		TH1D* h_mass_HLTv4p2_noCorr = new TH1D("h_mass_OS_HLTV4p2_noCorr_"+Tag[i_tup], "", 10000, 0, 10000);
		TH1D* h_mass_HLTv4p3_noCorr = new TH1D("h_mass_OS_HLTV4p3_noCorr_"+Tag[i_tup], "", 10000, 0, 10000);
			...
			if( ntuple->isTriggered( analyzer->HLT ) && GenFlag)
			{
				...
				if( isPassEventSelection == kTRUE )
				{
					...
					if( isMC )
					{
						...
						h_mass_HLTv4p2_noCorr->Fill( reco_M, GenWeight*PUWeight	);
						h_mass_HLTv4p3_noCorr->Fill( reco_M, GenWeight*PUWeight	);
					...
					{
						if( ntuple->runNum < 257933 ) h_mass_HLTv4p2->Fill( reco_M );
						else                          h_mass_HLTv4p3->Fill( reco_M );
					}
				}
		...
		f->cd();
		h_mass_HLTv4p2_noCorr->Write();
		h_mass_HLTv4p3_noCorr->Write();
		h_mass_HLTv4p2->Write();
		h_mass_HLTv4p3->Write();

		h_mass_HLTv4p2_noCorr->Sumw2();
		h_mass_HLTv4p3_noCorr->Sumw2();
		...
		TH1D* h_mass_HLTv4p2_noCorr_Norm = (TH1D*)h_mass_HLTv4p2_noCorr->Clone( "h_mass_OS_HLTv4p2_noCorr_Norm_"+Tag[i_tup] );
		h_mass_HLTv4p2_noCorr_Norm->Scale( Norm_HLTv4p2 );

		TH1D* h_mass_HLTv4p3_noCorr_Norm = (TH1D*)h_mass_HLTv4p3_noCorr->Clone( "h_mass_OS_HLTv4p3_noCorr_Norm_"+Tag[i_tup] );
		h_mass_HLTv4p3_noCorr_Norm->Scale( Norm_HLTv4p3 );

		h_mass_HLTv4p2_noCorr_Norm->Write();
		h_mass_HLTv4p3_noCorr_Norm->Write();
