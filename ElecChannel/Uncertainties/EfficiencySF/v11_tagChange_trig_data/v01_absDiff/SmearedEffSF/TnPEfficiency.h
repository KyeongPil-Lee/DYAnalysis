...
private:
  ...
  {
    cout << "[TnPEfficiency::FillDiffArray] the difference = absolute size (ignore sign info.)" << endl;
    ...
    for(Int_t i_eta = 0; i_eta < nEtaBin; i_eta++)
    {
      for(Int_t i_pt = 0; i_pt < nPtBin; i_pt++)
      {
        ...
        {
          diff_reco_data_[i_eta][i_pt] = fabs(h_relDiff_reco->GetBinContent(i_etaBin, i_ptBin) * eff_reco_data_[i_eta][i_pt]);
          diff_ID_data_[i_eta][i_pt]   = fabs(h_relDiff_ID->GetBinContent(i_etaBin, i_ptBin)   * eff_ID_data_[i_eta][i_pt]);
        }
        else if( uncType == "nlo" ) // -- MC variation, including trigger
        {
          diff_reco_MC_[i_eta][i_pt] = fabs(h_relDiff_reco->GetBinContent(i_etaBin, i_ptBin) * eff_reco_MC_[i_eta][i_pt]);
          diff_ID_MC_[i_eta][i_pt]   = fabs(h_relDiff_ID->GetBinContent(i_etaBin, i_ptBin)   * eff_ID_MC_[i_eta][i_pt]);
          diff_trig_MC_[i_eta][i_pt] = fabs(h_relDiff_trig->GetBinContent(i_etaBin, i_ptBin) * eff_trig_MC_[i_eta][i_pt]);
        }
        else if( uncType == "tagChange" ) // -- MC variation for reco+ID & data variation for trigger
        {
          diff_reco_MC_[i_eta][i_pt]   = fabs(h_relDiff_reco->GetBinContent(i_etaBin, i_ptBin) * eff_reco_MC_[i_eta][i_pt]);
          diff_ID_MC_[i_eta][i_pt]     = fabs(h_relDiff_ID->GetBinContent(i_etaBin, i_ptBin)   * eff_ID_MC_[i_eta][i_pt]);
          diff_trig_data_[i_eta][i_pt] = fabs(h_relDiff_trig->GetBinContent(i_etaBin, i_ptBin) * eff_trig_data_[i_eta][i_pt]);
        }
      }
    }
  }
