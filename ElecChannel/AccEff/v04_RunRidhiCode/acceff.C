...
void acceff() {
  ...
  double sumofWts[11] = {7.58772E+11,1.44504E+11,2.19885E+08,7.00873E+06,122990,69998.9,11081,9415.63,4893.46,664.034,159.616}; // -- calculated by kP
  
  // workdir = "/tmp/rchawla/eos/cms/store/group/phys_higgs/cmshww/arun/DYAnalysis_76X_Calibrated/DY_Signal/";
  workdir = gSystem->Getenv("KP_DATA_PATH");
  workdir = workdir + "/";
  ...
  for(unsigned int j = 0; j < InputFiles_signal_DY.size(); ++j ) {
    ...
    TFile *f1 = TFile::Open(fileName_data);
    TFile *f2 = TFile::Open(fileName_MC);
