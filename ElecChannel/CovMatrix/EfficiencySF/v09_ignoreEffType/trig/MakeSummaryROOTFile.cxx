...
void MakeSummaryROOTFile(TString effType, TString theUncType)
{
  cout << "[MakeSummaryROOTFile] effType = " << effType << ", the unc. type = " << theUncType << endl;
  
  TString fileName_output = TString::Format("ROOTFile_CovM_EfficiencySF_ignore%s_in_%s.root", effType.Data(), theUncType.Data());
...
void MakeSummaryROOTFile()
{
  ...
  // MakeSummaryROOTFile(effType, "bkgChange"); // -- no uncertainty from the background shape for the trigger eff.
  MakeSummaryROOTFile(effType, "tagChange");
}