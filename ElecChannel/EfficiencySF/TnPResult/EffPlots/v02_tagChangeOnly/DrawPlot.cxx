...
public:
  ...
  vector<TString> vec_uncType_ = {"tagChange"};
  // vector<TString> vec_uncType_mc_   = {"tagChange", "nlo"};
  vector<TString> vec_uncType_mc_   = {"tagChange"};
  ...
  void Produce()
  {
    ...
    DrawDiffPlot_AllPtRange("tagChange");
  }
...
private:
  ...
  void DrawEffPlot_CV_vs_Alt_PtRange(TString dataType, Double_t minPt, Double_t maxPt )
  {
    ...
    if( effType_ == "trig" ) canvas->SetRangeY(0.7, 1.1);
    if( effType_ == "trig" && minPt == 10 ) canvas->SetRangeY(0, 1.1);
    if( effType_ == "trig" && minPt == 20 ) canvas->SetRangeY(0, 1.1);
