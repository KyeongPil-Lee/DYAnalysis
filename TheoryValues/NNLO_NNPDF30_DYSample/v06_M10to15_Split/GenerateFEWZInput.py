...
class Generator:
	def __init__( self ):
		...
		self.photonPtMin = 10.0
		self.photonEtaMax = 2.5
		self.jetPtMin = 20.0
		self.jetEtaMax = 4.5
	...
	def SetRelUnc( self, value ):
		self.relUnc = value
		self.absUnc = 0.0

	def SetAbsUnc( self, value ):
		self.relUnc = 0.0
		self.absUnc = value

	def SetZpoleFocus( self, value ):
		self.ZpoleFocus = value
	...
	def SetJetPtMin( self, value ):
		self.jetPtMin = value

	def SetJetEtaMax( self, value ):
		self.jetEtaMax = value

	def SetPhotonPtMin( self, value ):
		self.photonPtMin = value

	def SetPhotonEtaMax( self, value ):
		self.photonEtaMax = value
	...
	def GenerateParameterInput( self ):
		...
		scale = (self.massMin + self.massMax) / 2.0
		if self.massMin == 50 and self.massMax == 1000000: scale = 91.1876
		if self.massMin == 60 and self.massMax == 120: scale = 91.1876
...
=============================================
'Factorization scale  (GeV)    = ' {_scale:.1f}d0
'Renormalization scale  (GeV)  = ' {_scale:.1f}d0
=============================================
...
'Absolute accuracy                  = ' {_absUnc:.8f}d0
...
'Z pole focus (1=Yes, 0=No)	= ' {_ZpoleFocus:.0f}
...
'Minimum pT for observable photon   = ' {_photonPtMin}d0
'Maximum eta for observable photon  = ' {_photonEtaMax}d0
PHOTON CUTS--------------------------------------
...
'Minimum pT for observable jets     = ' {_jetPtMin}d0
'Maximum eta for observable jets    = ' {_jetEtaMax}d0
JET CUTS--------------------------------------
...
=============================================""".format( 
		...
		_relUnc=self.relUnc, _absUnc=self.absUnc, _pQCDOrder=self.pQCDOrder, _ZpoleFocus = self.ZpoleFocus, _EWControl=self.EWControl, 
		...
		_photonPtMin=self.photonPtMin, _photonEtaMax=self.photonEtaMax, 
		_jetPtMin=self.jetPtMin, _jetEtaMax=self.jetEtaMax,
		_PDFSet=self.PDFSet )
		)
		f.close()
...
'1.  Z/W pT           ' 20		{_diPtMin:.0f}d0		{_diPtMax:.0f}d0		1
...
'Display option =' 0
""".format(
		_diPtMin=self.diPtMin, _diPtMax=self.diPtMax,
		_diRapMin=self.diRapMin, _diRapMax=self.diRapMax,
		_massMin=self.massMin, _massMax=self.massMax )
		)
		f_hist.close()
...
class GeneratorForDYSample:
	def __init__( self ):
		self.List_massBinEdge = [10, 11, 12, 13, 14, 15]
		self.List_xSecNLO = [ 18610.0/3.0, 18610.0/3.0, 18610.0/3.0, 18610.0/3.0, 18610.0/3.0 ]
		self.List_AbsUnc = []
		for xSecNLO in self.List_xSecNLO:
			self.List_AbsUnc.append( 0.001 * xSecNLO )

		self.generator = Generator()
		self.generator.SetPDFSet( "NNPDF30_nnlo_as_0118" )
		self.generator.SetpQCDOrder( 2 )
		self.generator.SetnCore( 24 )
		self.generator.SetEWControl( 1 ) # -- no FSR -- #

		self.generator.SetJetPtMin( 0.0 )
		self.generator.SetJetEtaMax( 100000 )
		self.generator.SetPhotonPtMin( 0.0 )
		self.generator.SetPhotonEtaMax( 100000 )

	def GenerateInputs_All( self ):
		for i in range(0, len(self.List_massBinEdge)-1):
			lowerEdge = self.List_massBinEdge[i]
			upperEdge = self.List_massBinEdge[i+1]
			absUnc = self.List_AbsUnc[i]

			self.GenerateInputs_EachMassRange( lowerEdge, upperEdge, absUnc )

		# -- for x-check -- #
		# self.GenerateInputs_EachMassRange( 50, 1000000, self.List_AbsUnc[1] )
		# self.GenerateInputs_EachMassRange( 60, 120, self.List_AbsUnc[1] )


	def GenerateInputs_EachMassRange( self, lowerEdge, upperEdge, absUnc ):
		tag = "M%.0lfto%.0lf" % (lowerEdge, upperEdge)
		self.generator.SetTag( tag )

		self.generator.SetZpoleFocus( 0 )
		if lowerEdge == 50 and upperEdge == 100: self.generator.SetZpoleFocus( 1 )
		if lowerEdge == 50 and upperEdge == 1000000: self.generator.SetZpoleFocus( 1 )
		if lowerEdge == 60 and upperEdge == 120: self.generator.SetZpoleFocus( 1 )

		self.generator.SetMassRange( lowerEdge, upperEdge )
		self.generator.SetAbsUnc( absUnc )

		self.generator.GenerateParameterInput()
		self.GenerateHistogramInputForDYSample( lowerEdge, upperEdge )
		self.generator.GenerateBatchJobScript()


	def GenerateHistogramInputForDYSample( self, massMin, massMax ):

		if massMax == 1000000: massMax = 1050
...
Histogram bin display (0 = bin central value, -1 = bin low edge, 1 = bin upper edge)
'Display option =' 0
""".format(_massMin=massMin, _massMax=massMax ))
		f_hist.close()

# -- main part -- #
gen = GeneratorForDYSample()
gen.GenerateInputs_All()
